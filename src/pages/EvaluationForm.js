...
export default function EvaluationForm() {
  ...
  const existing = savedData.find((e) => e.id === id);
  ...
  const saveData = useCallback((status, data = formData) => {
  const existingData = JSON.parse(localStorage.getItem("evaluateesStatus")) || [];
  const updatedData = existingData.filter(e => e.id !== id);
  updatedData.push({ id, status, feedbackSaved: data });
  localStorage.setItem("evaluateesStatus", JSON.stringify(updatedData));
}, [id, formData]);

  useEffect(() => {
    if (!existing || existing.status !== "completed") {
      saveData("inprogress", formData);
    }
  }, [existing, formData, saveData]);
  ...
  return (
    <div className="form-container">
      <div className="header">
        <div className="header-left">TAOKAE Project 360 Degree Feedback</div>
